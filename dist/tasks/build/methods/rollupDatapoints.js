...
async function findAndTransform(spec, {
  datapointService,
  logger
}) {
  /*
  ...
  while (begTime < endTime) {
    ...
    logger.info('Find and transform', {
      query
...
function optionsForRollup(rollup, {
  big_math: big
}) {
  const aggregation = rollup.aggregations.reduce((agg, {
    alias,
    args,
    field,
    filter,
    ...
    if (typeof filter !== 'undefined') {
      ...
      if (!filterFn) throw new Error('Unknown aggregation filter'); // The filter is always the last arg
